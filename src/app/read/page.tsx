...
export default function ReadListPage() {
  ...
  return (
    <div className="min-h-dvh bg-cream">
      <AppNav subtitle={`共 ${readBooks.length} 篇精读 / 深读`} />
      ...
      <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6">
        {readBooks.length === 0 ? (
          <div className="flex flex-col items-center py-16 text-center">
            ...
            <p className="mt-2 text-sm text-ink-muted">从发现页找一个困惑，开启你的第一本书</p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/discover"
                className="rounded-full bg-accent px-6 py-3 text-sm text-white hover:bg-accent-dark"
              >
                去发现
              </Link>
              <Link
                href="/chat"
                className="rounded-full border border-paper bg-white px-6 py-3 text-sm text-ink-muted hover:border-accent hover:text-accent"
              >
                去对话
              </Link>
            </div>
